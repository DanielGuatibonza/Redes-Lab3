...
log.debug('Archivo a recibir: file' + file_size_MB + '.txt, Tamaño: ' + file_size_MB + 'MB.')
...
def recibir_archivo(i):
    ...
    log.debug('El tiempo de transferencia del archivo al cliente ' + str(s.getsockname()) + ' fue de ' + str(time.time() - start_time) + ' segundos.')
    ...
    if len(file_data)==file_size and hash_data == hash_server:
        ...
        s.sendto(ARCHIVO_RECIBIDO.encode(), server_address)
    else:
        s.sendto(HASH_INCORRECTO.encode(), server_address)
